...
class Mqtt3UnsubAckDecoderTest extends AbstractMqtt3DecoderTest {
    ...
    void decode_ERROR_CASES(final int errorcase) throws Exception {
        final byte[] encoded = switch (errorcase) {
            case 1 -> Bytes.concat(MALFORMED_UNSUBACK_BEGIN_WRONG_FLAGS, MAX_PACKET_ID);
            case 2 -> Bytes.concat(MALFORMED_UNSUBACK_BEGIN_TOO_LONG_LENGTH, MIN_PACKET_ID, ENDING_TOO_LONG_MALFORMED);
