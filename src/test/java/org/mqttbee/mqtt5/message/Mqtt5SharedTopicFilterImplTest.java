...
public class Mqtt5SharedTopicFilterImplTest {
    ...
    public Mqtt5SharedTopicFilterImplTest(final int source) {
        this.source = source;
    }
    ...
    private Mqtt5TopicFilterImpl from(final String shareName, final String topicFilter) {
        final String string = "$share/" + shareName + "/" + topicFilter;
        switch (source) {
            case FROM_SHARE_NAME_AND_TOPIC_FILTER:
                return Mqtt5SharedTopicFilterImpl.from(shareName, topicFilter);
            case FROM_BYTE_BUF:
                final ByteBuf byteBuf = Unpooled.buffer();
                final byte[] binary = string.getBytes(Charset.forName("UTF-8"));
                byteBuf.writeShort(binary.length);
    ...
    public void test_shared() {
        ...
        final Mqtt5TopicFilterImpl mqtt5TopicFilter = from(shareName, topicFilter);
        assertNotNull(mqtt5TopicFilter);
        assertTrue(mqtt5TopicFilter.isShared());
        assertTrue(mqtt5TopicFilter instanceof Mqtt5SharedTopicFilterImpl);

        final Mqtt5SharedTopicFilterImpl mqtt5SharedTopicFilter = (Mqtt5SharedTopicFilterImpl) mqtt5TopicFilter;
    ...
    public void test_shared_share_name_must_not_contain_wildcards() {
        ...
        final String topicFilter = "abc/def";
        final Mqtt5TopicFilterImpl mqtt5TopicFilter1 = from(shareName1, topicFilter);
        final Mqtt5TopicFilterImpl mqtt5TopicFilter2 = from(shareName2, topicFilter);
        final Mqtt5TopicFilterImpl mqtt5TopicFilter3 = from(shareName3, topicFilter);
        final Mqtt5TopicFilterImpl mqtt5TopicFilter4 = from(shareName4, topicFilter);
    ...
    public void test_shared_topic_filter_must_be_case_sensitive() {
        ...
        final Mqtt5TopicFilterImpl mqtt5TopicFilter1 = from(shareName, topicFilter1);
        final Mqtt5TopicFilterImpl mqtt5TopicFilter2 = from(shareName, topicFilter2);
        assertNotNull(mqtt5TopicFilter1);
        assertNotNull(mqtt5TopicFilter2);
        assertTrue(mqtt5TopicFilter1.isShared());
        assertTrue(mqtt5TopicFilter1 instanceof Mqtt5SharedTopicFilterImpl);
        assertTrue(mqtt5TopicFilter2.isShared());
        assertTrue(mqtt5TopicFilter2 instanceof Mqtt5SharedTopicFilterImpl);
        assertNotEquals(mqtt5TopicFilter1.toString(), mqtt5TopicFilter2.toString());

        final Mqtt5SharedTopicFilterImpl mqtt5SharedTopicFilter1 = (Mqtt5SharedTopicFilterImpl) mqtt5TopicFilter1;
        final Mqtt5SharedTopicFilterImpl mqtt5SharedTopicFilter2 = (Mqtt5SharedTopicFilterImpl) mqtt5TopicFilter2;
    ...
    public void test_shared_share_name_must_not_contain_topic_level_separator() {
        if (source == FROM_BYTE_BUF) {
            ...
            final String topicFilter = "abc/def/ghi";
            final Mqtt5SharedTopicFilterImpl mqtt5SharedTopicFilter =
                    Mqtt5SharedTopicFilterImpl.from(shareName, topicFilter);
