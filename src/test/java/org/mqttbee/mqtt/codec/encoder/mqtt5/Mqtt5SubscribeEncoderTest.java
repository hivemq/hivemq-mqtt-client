...
class Mqtt5SubscribeEncoderTest extends AbstractMqtt5EncoderWithUserPropertiesTest {
    ...
    void encode_allProperties() {
        ...
        final MqttUserPropertiesImpl userProperties =
                MqttUserPropertiesImpl.of(ImmutableList.of(mqtt5UserProperty));
    ...
    void encode_subscriptionOptionsQos(final MqttQoS qos) {
        ...
        final MqttSubscribe subscribe =
                new MqttSubscribe(subscriptions, MqttUserPropertiesImpl.NO_USER_PROPERTIES);
        encode(expected, subscribe, 10);
    }
        ...
        final ImmutableList<MqttSubscription> subscriptions =
                ImmutableList.of(
                        new MqttSubscription(
                                ...
                                DEFAULT_RETAIN_AS_PUBLISHED));
        final MqttSubscribe subscribe =
                new MqttSubscribe(subscriptions, maxPacket.getTooManyUserProperties(1));
        encode(expected, subscribe, 10);
    }
    ...
    void encode_maximumPacketSizeExceeded_throws() {
        ...
        final int numberOfMaxTopicFilters =
                (MAXIMUM_PACKET_SIZE_LIMIT - headerLength)
                        / (UTF8_LENGTH_ENCODED
                                + UTF8_STRING_MAX_LENGTH
                                + SUBSCRIPTION_OPTIONS_LENGTH);
        final int leftoverLength =
                (MAXIMUM_PACKET_SIZE_LIMIT - headerLength)
                        % (UTF8_LENGTH_ENCODED
                                + UTF8_STRING_MAX_LENGTH
                                + SUBSCRIPTION_OPTIONS_LENGTH);
        ...
        final char[] leftoverTopicBytes =
                new char[leftoverLength - SUBSCRIPTION_OPTIONS_LENGTH + 1];
        ...
        final MqttQoS qos = MqttQoS.AT_LEAST_ONCE;
        final List<MqttSubscription> maxSizeSubscriptions =
                Collections.nCopies(
                        numberOfMaxTopicFilters,
                        new MqttSubscription(
                                requireNonNull(topicFilter),
                                qos,
                                DEFAULT_NO_LOCAL,
                                DEFAULT_RETAIN_HANDLING,
                                DEFAULT_RETAIN_AS_PUBLISHED));
        ...
        final List<MqttSubscription> leftoverSubscription =
                ImmutableList.of(
                        (new MqttSubscription(
                                requireNonNull(leftoverTopicFilter),
                                qos,
                                DEFAULT_NO_LOCAL,
                                DEFAULT_RETAIN_HANDLING,
                                DEFAULT_RETAIN_AS_PUBLISHED)));

        final Iterable<MqttSubscription> subscriptions =
                Iterables.concat(maxSizeSubscriptions, leftoverSubscription);

        final MqttSubscribe subscribe =
                new MqttSubscribe(
                        ImmutableList.copyOf(subscriptions),
                        MqttUserPropertiesImpl.NO_USER_PROPERTIES);

        final int packetIdentifier = 2;
        final MqttStatefulSubscribe subscribeInternal =
                subscribe.createStateful(
                        packetIdentifier, MqttStatefulSubscribe.DEFAULT_NO_SUBSCRIPTION_IDENTIFIER);

        final Throwable exception =
                assertThrows(
                        MqttMaximumPacketSizeExceededException.class,
    ...
    private void encode(
            final byte[] expected, final MqttSubscribe subscribe, final int packetIdentifier) {
        final MqttStatefulSubscribe subscribeInternal =
                subscribe.createStateful(
                        packetIdentifier, MqttStatefulSubscribe.DEFAULT_NO_SUBSCRIPTION_IDENTIFIER);
        encodeInternal(expected, subscribeInternal);
    ...
    private void encodeInternal(
            final byte[] expected, final MqttStatefulSubscribe subscribeInternal) {
