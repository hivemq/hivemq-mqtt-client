...
class Mqtt3PubAckDecoderTest extends AbstractMqtt3DecoderTest {
  ...
  void decode_SUCESS(final boolean useMaxPacketId) {
    ...
    final byte[] encoded =
        Bytes.concat(WELLFORMED_PUBACK_BEGIN, useMaxPacketId ? MAX_PACKET_ID : MIN_PACKET_ID);
  ...
  void decode_ERROR_TOO_LONG() {
    ...
    final byte[] encoded =
        Bytes.concat(
            MALFORMED_PUBACK_BEGIN_TOO_LONG_LENGTH, MAX_PACKET_ID, ENDING_TOO_LONG_MALFORMED);
    final ByteBuf byteBuf = channel.alloc().buffer();
    byteBuf.writeBytes(encoded);
    channel.writeInbound(byteBuf);
    final MqttPubAck pubAck = channel.readInbound();
