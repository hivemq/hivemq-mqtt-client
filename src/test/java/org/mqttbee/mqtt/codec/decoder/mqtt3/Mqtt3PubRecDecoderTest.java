...
class Mqtt3PubRecDecoderTest extends AbstractMqtt3DecoderTest {
  ...
  void decode_SUCESS(final boolean useMaxPacketId) {
    final byte[] encoded =
        Bytes.concat(WELLFORMED_PUBREC_BEGIN, useMaxPacketId ? MAX_PACKET_ID : MIN_PACKET_ID);
  ...
  void decode_ERROR_CASES(final int errorcase) throws Exception {
    ...
    switch (errorcase) {
      ...
      case 2:
        encoded =
            Bytes.concat(
                MALFORMED_PUBREC_BEGIN_TOO_LONG_LENGTH, MIN_PACKET_ID, ENDING_TOO_LONG_MALFORMED);
        break;
      default:
        throw new Exception();
    }
