...
class Mqtt3PublishDecoderTest extends AbstractMqtt3DecoderTest {
  ...
  private ByteBuf createWellformedPublish(
    ...
    if (remainingLength > 1024) {
      throw new Exception(); // too avoid numbers which must be represented with a variable byte
      // integer. (Of course the limit could be much greater than 1024)
    }
  ...
  void decode_SUCCESS(final boolean retained, final boolean isDup, final int qos) throws Exception {
    ...
    final ByteBuf byteBuf =
        createWellformedPublish(isDup, qos, retained, 1, topic.getBytes(), payload.getBytes());
    ...
    assertArrayEquals(
        payload.getBytes(),
        ByteBufferUtil.getBytes(publishInternal.getWrapped().getPayload().get()));
    assertEquals(isDup, publishInternal.isDup());
    assertEquals(qos, publishInternal.getWrapped().getQos().getCode());
    if (qos == 0) {
      assertEquals(
          MqttPublishWrapper.NO_PACKET_IDENTIFIER_QOS_0, publishInternal.getPacketIdentifier());
    } else {
