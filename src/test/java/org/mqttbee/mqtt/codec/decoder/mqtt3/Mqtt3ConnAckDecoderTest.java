...
class Mqtt3ConnAckDecoderTest extends AbstractMqtt3DecoderTest {
  ...
  void decode_SUCCESS(final boolean sessionPresent) {
    ...
    assertEquals(
        Mqtt5ConnAckReasonCode.SUCCESS, connAck.getReasonCode()); // Mqtt3ConnAckReturnCode.SUCCESS
  ...
  void decode_ERROR_CASES(final int errorcase) throws Exception {
    ...
    switch (errorcase) {
      case 1:
        encoded =
            Bytes.concat(
                MALFORMED_CONNACK_BEGIN_WORNG_FLAGS, SESSION_PRESENT_FALSE, REASON_CODE_BAD);
        break;
      case 2:
        encoded =
            Bytes.concat(
