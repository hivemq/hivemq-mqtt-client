...
class Mqtt3UnsubAckDecoderTest {
    ...
    @BeforeEach
    void setUp() {
        channel = new EmbeddedChannel(new Mqtt3Decoder(new Mqtt3UnsubAckTestMessageDecoders()));
    }
    ...
    void decode_SUCESS(boolean useMaxPacketId) {
        ...
        final Mqtt3UnsubAckImpl unsubAck = channel.readInbound();
        assertNotNull(unsubAck);
        assertEquals(useMaxPacketId ? 65535 : 0, unsubAck.getPacketId());
    }
    ...
    void decode_ERROR_CASES(int errorcase) throws Exception {
        ...
        byteBuf.writeBytes(encoded);
        channel.writeInbound(byteBuf);
        final Mqtt3UnsubAck unsubAck = channel.readInbound();
        assertFalse(channel.isOpen());
        assertNull(unsubAck);
    }

    private static class Mqtt3UnsubAckTestMessageDecoders implements Mqtt3MessageDecoders {
        @Nullable
        @Override
