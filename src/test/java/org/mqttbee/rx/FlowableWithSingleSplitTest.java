...
class FlowableWithSingleSplitTest {
    ...
    void split() {
        ...
        final FlowableWithSingleSplit<? extends CharSequence, StringBuilder, String> flowableWithSingleSplit =
                new FlowableWithSingleSplit<>(flowable, StringBuilder.class, String.class);

        final AtomicInteger count = new AtomicInteger();
        flowableWithSingleSplit.doOnSingle(
                (stringBuilder, subscription) -> assertEquals("single", stringBuilder.toString()))
    ...
    void observeOnBoth() {
        ...
        final ExecutorService executorService =
                Executors.newFixedThreadPool(1, new ThreadFactoryBuilder().setNameFormat("test_thread").build());

        final AtomicInteger count = new AtomicInteger();
        flowableWithSingleSplit.observeOnBoth(Schedulers.from(executorService))
                .doOnSingle((stringBuilder, subscription) -> {
    ...
    void observeOnFlowable() {
        ...
        final AtomicInteger count = new AtomicInteger();
        flowableWithSingleSplit.doOnSingle((stringBuilder, subscription) -> {
            assertEquals("single", stringBuilder.toString());
    ...
    void mapBoth() {
        ...
        flowableWithSingleSplit.mapBoth(stringBuilder -> (double) stringBuilder.toString().length(), String::length)
