...
public class Mqtt5ClientImpl implements Mqtt5Client {
  ...
  public Flowable<Mqtt5Publish> remainingPublishes() {
    return new MqttGlobalIncomingPublishFlowable(
            MqttGlobalIncomingPublishFlow.TYPE_REMAINING_PUBLISHES, clientData)
  ...
  public Flowable<Mqtt5Publish> allPublishes() {
    return new MqttGlobalIncomingPublishFlowable(
            MqttGlobalIncomingPublishFlow.TYPE_ALL_PUBLISHES, clientData)
        .cast(Mqtt5Publish.class)
        .observeOn(clientData.getExecutorConfig().getRxJavaScheduler()); // TODO temp
  }
  ...
  public Single<Mqtt5UnsubAck> unsubscribe(@NotNull final Mqtt5Unsubscribe unsubscribe) {
    ...
    return Single.<Mqtt5UnsubAck>create(
            emitter -> {
              final MqttClientConnectionData clientConnectionData =
                  clientData.getRawClientConnectionData();
              if (clientConnectionData == null) {
                ...
                subscriptionHandler.unsubscribe(
                    new MqttUnsubscribeWithFlow(mqttUnsubscribe, emitter));
              }
            })
        .observeOn(clientData.getExecutorConfig().getRxJavaScheduler());
  }
    ...
    return Completable.create(
            emitter -> {
              final MqttClientConnectionData clientConnectionData =
                  clientData.getRawClientConnectionData();
              if (clientConnectionData != null) {
                MqttDisconnectUtil.disconnect(clientConnectionData.getChannel(), mqttDisconnect)
                    .addListener(
                        future -> {
                          if (future.isSuccess()) {
                            emitter.onComplete();
