...
/**
 * Util for sending a DISCONNECT message or channel closing without sending a DISCONNECT message
 * from the client side. Fires {@link ChannelCloseEvent}s.
 *
 * @author Silvio Giebl
 */
public class MqttDisconnectUtil {
   ...
   */
  public static ChannelFuture disconnect(
      @NotNull final Channel channel, @NotNull final MqttDisconnect disconnect) {
    fireChannelCloseEvent(
        channel, new Mqtt5MessageException(disconnect, "DISCONNECT through API"), false);
  ...
  static ChannelFuture disconnectAndClose(
      @NotNull final Channel channel, @NotNull final MqttDisconnect disconnect) {
