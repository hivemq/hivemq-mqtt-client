...
public class MqttSubscriptionHandler extends ChannelInboundHandlerAdapter {
  ...
  MqttSubscriptionHandler(
      final MqttIncomingPublishFlows subscriptionFlows,
      @Named("incomingPublish") final Scheduler.Worker worker,
  ...
  private void writeSubscribe(
    ...
    ctx.writeAndFlush(subscribeWrapperWithFlow.getSubscribe())
        .addListener(
            future -> {
              if (!future.isSuccess()) {
  ...
  private void writeUnsubscribe(
    ...
    final MqttUnsubscribeWrapperWithFlow unsubscribeWrapperWithFlow =
        unsubscribeWithFlow.wrap(packetIdentifier);
    ...
    ctx.writeAndFlush(unsubscribeWrapperWithFlow.getUnsubscribe())
        .addListener(
            future -> {
              if (!future.isSuccess()) {
  ...
  private void handleSubAck(
    ...
    if (allErrorCodes(subAck.getReasonCodes())) {
      worker.schedule(
          () ->
              flow.onError(new Mqtt5MessageException(subAck, "SUBACK contains only Error Codes")));
    } else {
  ...
  private void handleUnsubAck(
    ...
    if (allErrorCodes(unsubAck.getReasonCodes())) {
      worker.schedule(
          () ->
              flow.onError(
                  new Mqtt5MessageException(unsubAck, "UNSUBACK contains only Error Codes")));
  ...
  private static boolean allErrorCodes(
      @NotNull final ImmutableList<? extends Mqtt5ReasonCode> reasonCodes) {
