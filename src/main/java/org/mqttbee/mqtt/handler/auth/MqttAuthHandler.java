 ...
 *
 * <p>After successful connection the handler replaces itself with a {@link MqttReAuthHandler}.
 *
 * @author Silvio Giebl
 */
@ChannelScope
public class MqttAuthHandler extends AbstractMqttAuthHandler
     ...
     *
     * <ul>
     *   <li>{@link Mqtt5EnhancedAuthProvider#onAuthSuccess(Mqtt5ClientData, Mqtt5ConnAck)},
     *   <li>{@link Mqtt5EnhancedAuthProvider#onAuthRejected(Mqtt5ClientData, Mqtt5ConnAck)} or
     *   <li>{@link Mqtt5EnhancedAuthProvider#onAuthError(Mqtt5ClientData, Throwable)}
     * </ul>
     *
     * have been called.
     */
     ...
     *
     * <ul>
     *   <li>Calls {@link Mqtt5EnhancedAuthProvider#onAuth(Mqtt5ClientData, Mqtt5Connect,
     *       Mqtt5EnhancedAuthBuilder)} which adds enhanced auth data to the CONNECT message.
     *   <li>Sends the CONNECT message with the enhanced auth data.
     * </ul>
     *
    ...
    private void writeConnect(
        ...
        enhancedAuthProvider
                .onAuth(clientData, connect, enhancedAuthBuilder)
                .whenCompleteAsync(
                        (aVoid, throwable) -> {
                            if (enhancedAuthProviderAccepted(throwable)) {
                                final MqttStatefulConnect statefulConnect =
                                        connect.createStateful(
                                                clientData.getRawClientIdentifier(),
                                                enhancedAuthBuilder.build());
     ...
     *
     * <ul>
     *   <li>Calls {@link Mqtt5EnhancedAuthProvider#onAuthRejected(Mqtt5ClientData, Mqtt5ConnAck)}
     *       and closes the channel if the CONNACK message contains an Error Code.
     *   <li>Sends a DISCONNECT message if the enhanced auth data of the CONNACK message is not
     *       valid.
     *   <li>Otherwise calls {@link Mqtt5EnhancedAuthProvider#onAuthSuccess(Mqtt5ClientData,
     *       Mqtt5ConnAck)}.
     *   <li>Sends a DISCONNECT message if the enhanced auth provider did not accept the enhanced
     *       auth data.
     * </ul>
     *
     * @param ctx the channel handler context.
    ...
    private void readConnAck(
        ...
        if (connAck.getReasonCode().isError()) {
            ...
            enhancedAuthProvider
                    .onAuthSuccess(clientData, connAck)
                    .whenCompleteAsync(
                            (accepted, throwable) -> {
                                if (!enhancedAuthProviderAccepted(accepted, throwable)) {
                                    MqttDisconnectUtil.disconnect(
                                            ...
                                            new Mqtt5MessageException(
                                                    connAck, "Server auth success not accepted"));
                                }
                            },
                            ctx.executor());
            done = true;
            ...
            ctx.pipeline()
                    .replace(
                            this,
                            MqttReAuthHandler.NAME,
                            ChannelComponent.get(ctx.channel()).reAuthHandler());
        }
    }
    ...
    private boolean validateConnAck(
            ...
            @NotNull final MqttConnAck connAck,
        ...
        if (!enhancedAuth.getMethod().equals(enhancedAuthProvider.getMethod())) {
            MqttDisconnectUtil.disconnect(
                    ...
                    Mqtt5DisconnectReasonCode.PROTOCOL_ERROR,
                    new Mqtt5MessageException(
                            connAck, "Auth method must be the same as in the CONNECT message"));
            return false;
        }
        return true;
    }
    ...
    void readAuthSuccess(
        ...
        MqttDisconnectUtil.disconnect(
                ...
                Mqtt5DisconnectReasonCode.PROTOCOL_ERROR,
                new Mqtt5MessageException(
                        auth, "Server must not send an AUTH message with the Reason Code SUCCESS"));
    }
    ...
    /**
     * Calls {@link Mqtt5EnhancedAuthProvider#onAuthError(Mqtt5ClientData, Throwable)} with the
     * cause why the channel was closed if auth is not {@link #done} yet.
     *
     * @param ctx the channel handler context.
    ...
    private void handleChannelCloseEvent(
            @NotNull final ChannelHandlerContext ctx,
            @NotNull final ChannelCloseEvent channelCloseEvent) {
        ...
        if (!done) {
            ...
            final Mqtt5EnhancedAuthProvider enhancedAuthProvider =
                    getEnhancedAuthProvider(clientData);
