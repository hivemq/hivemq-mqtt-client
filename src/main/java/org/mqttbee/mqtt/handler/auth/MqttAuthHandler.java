...
public class MqttAuthHandler extends AbstractMqttAuthHandler
   ...
   *
   * <ul>
   *   <li>Calls {@link Mqtt5EnhancedAuthProvider#onAuthRejected(Mqtt5ClientData, Mqtt5ConnAck)} and
   *       closes the channel if the CONNACK message contains an Error Code.
   *   <li>Sends a DISCONNECT message if the enhanced auth data of the CONNACK message is not valid.
   *   <li>Otherwise calls {@link Mqtt5EnhancedAuthProvider#onAuthSuccess(Mqtt5ClientData,
   *       Mqtt5ConnAck)}.
   *   <li>Sends a DISCONNECT message if the enhanced auth provider did not accept the enhanced auth
   *       data.
   * </ul>
   *
   * @param ctx the channel handler context.
  ...
  private void readConnAck(
    ...
    if (connAck.getReasonCode().isError()) {
      ...
      ctx.pipeline()
          .replace(
              this, MqttReAuthHandler.NAME, ChannelComponent.get(ctx.channel()).reAuthHandler());
    }
  }
  ...
  void readReAuth(
    ...
    MqttDisconnectUtil.disconnect(
        ...
        Mqtt5DisconnectReasonCode.PROTOCOL_ERROR,
        new Mqtt5MessageException(
            auth, "Server must not send an AUTH message with the Reason Code REAUTHENTICATE"));
  }
  ...
  /**
   * Calls {@link Mqtt5EnhancedAuthProvider#onAuthError(Mqtt5ClientData, Throwable)} with the cause
   * why the channel was closed if auth is not {@link #done} yet.
   *
   * @param ctx the channel handler context.
