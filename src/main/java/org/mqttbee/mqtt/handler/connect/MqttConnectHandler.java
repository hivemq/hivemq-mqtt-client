...
public class MqttConnectHandler extends ChannelInboundHandlerWithTimeout {
   ...
   *
   * <p>The MQTT message Decoder is added after the write succeeded as the server is not allowed to
   * send messages before the CONNECT is sent.
   *
        ...
        .addListener(
            future -> {
              if (future.isSuccess()) {
                ...
                ctx.pipeline()
                    .addFirst(MqttDecoder.NAME, ChannelComponent.get(ctx.channel()).decoder());
              } else {
  ...
  private void handleConnAck(@NotNull final MqttConnAck connAck, @NotNull final Channel channel) {
    if (connAck.getReasonCode().isError()) {
      ...
      if (validateConnack(connAck, channel)) {
        ...
        pipeline.addLast(
            MqttDisconnectOnConnAckHandler.NAME, channelComponent.disconnectOnConnAckHandler());
