...
public class Mqtt5PublishEncoder
    extends Mqtt5WrappedMessageEncoder<MqttPublish, MqttPublishWrapper> {

  public static final MqttWrappedMessageEncoderProvider<
          MqttPublish, MqttPublishWrapper, MqttPublishEncoderProvider>
      PROVIDER =
          new NewMqttWrappedMessageEncoderProvider<>(
              Mqtt5PublishEncoder::new, Mqtt5PublishWrapperEncoder.PROVIDER);
  ...
  int calculatePropertyLength() {
    ...
    propertyLength +=
        intPropertyEncodedLength(
            message.getRawMessageExpiryInterval(), MESSAGE_EXPIRY_INTERVAL_INFINITY);
  ...
  void encodeFixedProperties(@NotNull final ByteBuf out) {
    encodeIntProperty(
        ...
        message.getRawMessageExpiryInterval(),
  ...
  public static class Mqtt5PublishWrapperEncoder
      extends Mqtt5MessageWrapperEncoder<
          MqttPublishWrapper, MqttPublish, MqttPublishEncoderProvider, Mqtt5PublishEncoder> {
    ...
    private static final MqttMessageWrapperEncoderApplier<
            MqttPublishWrapper, MqttPublish, Mqtt5PublishEncoder>
        APPLIER =
            new ThreadLocalMqttMessageWrapperEncoderApplier<>(Mqtt5PublishWrapperEncoder::new);
    ...
    int additionalPropertyLength() {
      ...
      additionalPropertyLength +=
          shortPropertyEncodedLength(message.getTopicAlias(), DEFAULT_NO_TOPIC_ALIAS);
      ...
      for (int i = 0; i < subscriptionIdentifiers.length(); i++) {
        additionalPropertyLength +=
            variableByteIntegerPropertyEncodedLength(subscriptionIdentifiers.get(i));
      }
    ...
    private void encodeProperties(@NotNull final ByteBuf out, final int maximumPacketSize) {
      ...
      for (int i = 0; i < subscriptionIdentifiers.length(); i++) {
        encodeVariableByteIntegerProperty(
            SUBSCRIPTION_IDENTIFIER, subscriptionIdentifiers.get(i), out);
      }
    }
