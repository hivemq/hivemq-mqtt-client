 ...
 * @author Silvio Giebl
 */
abstract class Mqtt5MessageWithUserPropertiesEncoder<M extends MqttMessage>
  ...
  /**
   * Returns the byte count of the MQTT message without omitting any properties. Calculation is only
  ...
  /**
   * Returns the remaining length byte count of this MQTT message respecting the given maximum
   * packet size.
   *
   * <p>If the minimal encoded length of the MQTT message is bigger than the maximum packet size,
  ...
  final int remainingLength(final int maxPacketSize) {
    return (mustOmitProperties(maxPacketSize))
        ? remainingLengthFromOmittedProperties(omittedPropertiesLength(maxPacketSize))
        : remainingLength();
  }

  /**
   * Returns the remaining length byte count of the MQTT message without omitting any properties.
   * Calculation is only performed if necessary.
   *
  ...
  private int remainingLength() {
    if (remainingLength == -1) {
      remainingLength =
          calculateRemainingLength() + encodedPropertyLengthWithHeader(propertyLength());
    }
  ...
  /**
   * Calculates the remaining length byte count of the MQTT message if properties with the given
   * length are omitted.
   *
   * @param omittedPropertiesLength the length of the omitted properties.
   * @return the remaining length of the MQTT message if properties with the given length are
   *     omitted.
   */
  private int remainingLengthFromOmittedProperties(final int omittedPropertiesLength) {
    return remainingLength()
        - encodedPropertyLengthWithHeader(propertyLength())
        + encodedPropertyLengthWithHeader(
            propertyLengthFromOmittedProperties(omittedPropertiesLength));
  }

  /**
   * Returns the property length byte count of the MQTT message respecting the given maximum packet
   * size.
   *
   * <p>If the minimal encoded length of the MQTT message is bigger than the maximum packet, size
   * the returned value is unspecified.
  ...
  final int propertyLength(final int maxPacketSize) {
    return (mustOmitProperties(maxPacketSize))
        ? propertyLengthFromOmittedProperties(omittedPropertiesLength(maxPacketSize))
        : propertyLength();
  }

  /**
   * Returns the property length byte count of the MQTT message without omitting any properties.
   * Calculation is only performed if necessary.
   *
  ...
  /**
   * Calculates the property length byte count of the MQTT message if properties with the given
   * length are omitted.
   *
   * @param omittedPropertiesLength the length of the omitted properties.
   * @return the property length of the MQTT message if properties with the given length are
   *     omitted.
   */
  ...
  /**
   * Checks whether properties of the MQTT message must be omitted to fit the given maximum packet
   * size.
   *
   * @param maxPacketSize the maximum packet size.
   ...
   *
   * <p>If the minimal encoded length of the MQTT message is bigger than the maximum packet size,
   * the length of all omissible properties is returned.
   *
   * @param maxPacketSize the maximum packet size.
  ...
  /**
   * Encodes the omissible properties of the MQTT message if they must not be omitted due to the
   * given maximum packet size.
   *
   * @param maxPacketSize the maximum packet size.
  ...
  /**
   * Base class for encoders of MQTT messages with an omissible Reason String and omissible User
   * Properties.
   */
  abstract static class Mqtt5MessageWithReasonStringEncoder<
          T extends MqttMessageWithReasonString<T, P>, P extends MqttMessageEncoderProvider<T>>
  ...
  /**
   * Base class for encoders of MQTT messages with an omissible Reason Code, an omissible Reason
   * String and omissible User Properties. The Reason Code is omitted if it is the default and the
   * property length is 0.
   */
  abstract static class Mqtt5MessageWithOmissibleReasonCodeEncoder<
          T extends MqttMessageWithReasonCode<T, R, P>,
          R extends Mqtt5ReasonCode,
          P extends MqttMessageEncoderProvider<T>>
  ...
  /**
   * Base class for encoders of MQTT messages with an Packet Identifier, an omissible Reason Code,
   * an omissible Reason String and omissible User Properties. The Reason Code is omitted if it is
   * the default and the property length is 0.
   */
  abstract static class Mqtt5MessageWithIdAndOmissibleReasonCodeEncoder<
          T extends MqttMessageWithIdAndReasonCode<T, R, P>,
          R extends Mqtt5ReasonCode,
          P extends MqttMessageEncoderProvider<T>>
