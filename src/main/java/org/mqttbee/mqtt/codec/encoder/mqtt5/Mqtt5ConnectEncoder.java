...
public class Mqtt5ConnectEncoder
    extends Mqtt5WrappedMessageEncoder<MqttConnect, MqttConnectWrapper> {
  ...
  int calculatePropertyLength() {
    ...
    propertyLength +=
        intPropertyEncodedLength(
            message.getSessionExpiryInterval(), DEFAULT_SESSION_EXPIRY_INTERVAL);
    propertyLength +=
        booleanPropertyEncodedLength(
            message.isResponseInformationRequested(), DEFAULT_RESPONSE_INFORMATION_REQUESTED);
    propertyLength +=
        booleanPropertyEncodedLength(
            message.isProblemInformationRequested(), DEFAULT_PROBLEM_INFORMATION_REQUESTED);
    ...
    if (restrictions != MqttConnectRestrictions.DEFAULT) {
      propertyLength +=
          shortPropertyEncodedLength(
              restrictions.getReceiveMaximum(), Mqtt5ConnectRestrictions.DEFAULT_RECEIVE_MAXIMUM);
      propertyLength +=
          shortPropertyEncodedLength(
  ...
  private int calculateWillPropertyLength() {
    ...
    if (willPublish != null) {
      willPropertyLength =
          ((Mqtt5PublishEncoder) willPublish.getEncoder()).propertyLength()
              + intPropertyEncodedLength(
                  willPublish.getDelayInterval(), Mqtt5WillPublish.DEFAULT_DELAY_INTERVAL);
  ...
  public static class Mqtt5ConnectWrapperEncoder
      extends Mqtt5MessageWrapperEncoder<
          MqttConnectWrapper, MqttConnect, MqttMessageEncoderProvider<MqttConnectWrapper>,
          Mqtt5ConnectEncoder> {

    private static final MqttMessageWrapperEncoderApplier<
            MqttConnectWrapper, MqttConnect, Mqtt5ConnectEncoder>
        APPLIER =
            new ThreadLocalMqttMessageWrapperEncoderApplier<>(Mqtt5ConnectWrapperEncoder::new);
    ...
    private void encodeFixedHeader(@NotNull final ByteBuf out) {
      ...
      MqttVariableByteInteger.encode(
          remainingLength(MqttVariableByteInteger.MAXIMUM_PACKET_SIZE_LIMIT), out);
    }
    ...
    private void encodeProperties(@NotNull final ByteBuf out) {
      ...
      MqttVariableByteInteger.encode(
          propertyLength(MqttVariableByteInteger.MAXIMUM_PACKET_SIZE_LIMIT), out);
