...
public class Mqtt3SubAckDecoder implements MqttMessageDecoder {
    ...
    private static final int MIN_REMAINING_LENGTH =
            3; // 2 for the packetId + 1 for at least one Subscription
    ...
    public MqttSubAck decode(
        ...
        final ImmutableList.Builder<Mqtt3SubAckReturnCode> returnCodesBuilder =
                ImmutableList.builder();
        for (int i = 0; i < returnCodeCount; i++) {
            final Mqtt3SubAckReturnCode returnCode =
                    Mqtt3SubAckReturnCode.from(in.readUnsignedByte());
            if (returnCode == null) {
