...
public class MqttSharedTopicFilterImpl extends MqttTopicFilterImpl
  ...
  /**
   * Checks whether the given UTF-8 encoded byte array represents a Shared Topic Filter. This method
   * does not validate whether it represents a valid Shared Topic Filter but only whether it starts
   * with {@link #SHARE_PREFIX}.
   *
   ...
   * method does not validate whether it represents a valid Shared Topic Filter but only whether it
   * starts with {@link #SHARE_PREFIX}.
   *
   ...
   *
   * <p>This method does not validate {@link MqttTopicFilterImpl#containsMustNotCharacters(byte[])}
   * and {@link #isShared(byte[])}.
   *
   ...
   * @return the created Shared Topic Filter or null if the Share Name is not a valid Share Name or
   *     the Topic Filter is not a valid Topic Filter.
   */
  @Nullable
  ...
  private int getFilterCharStart() {
    if (filterCharStart == -1) {
      ...
      filterCharStart =
          string.indexOf(MqttTopicImpl.TOPIC_LEVEL_SEPARATOR, SHARE_PREFIX_LENGTH + 1) + 1;
    }
