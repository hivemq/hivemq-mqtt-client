...
public class MqttTopicFilterImpl extends MqttUTF8StringImpl implements MqttTopicFilter {
   ...
   * @return the created Topic Filter or null if the byte array does not contain a well-formed Topic
   *     Filter.
   */
  @Nullable
   ...
   * @return the created Topic Filter or null if the byte buffer does not contain a well-formed
   *     Topic Filter.
   */
  @Nullable
   ...
   * @return a combination of {@link #WILDCARD_FLAG_MULTI_LEVEL} and {@link
   *     #WILDCARD_FLAG_SINGLE_LEVEL} indicating that a multi-level and/or single-level wildcards
   *     are present in the byte array or {@link #WILDCARD_CHECK_FAILURE} if the wildcard characters
   *     are misplaced
   */
   ...
   * @return a combination of {@link #WILDCARD_FLAG_MULTI_LEVEL} and {@link
   *     #WILDCARD_FLAG_SINGLE_LEVEL} indicating that a multi-level and/or single-level wildcards
   *     are present in the string or {@link #WILDCARD_CHECK_FAILURE} if the wildcard characters are
   *     misplaced
   */
  ...
  private static boolean matches(
      @NotNull final byte[] filter, final int offset, @NotNull final byte[] topic) {
    int fi = offset;
    int ti = 0;
    while (fi < filter.length) {
      ...
      if (fb == MULTI_LEVEL_WILDCARD) {
        ...
        if (ti == topic.length) {
          ...
          return (fb == MqttTopic.TOPIC_LEVEL_SEPARATOR)
              && (fi + 1 == filter.length)
  ...
  public boolean matches(@NotNull final MqttTopicFilter topicFilter) {
    return matches(
        MustNotBeImplementedUtil.checkNotImplemented(topicFilter, MqttTopicFilterImpl.class));
  }

  public boolean matches(@NotNull final MqttTopicFilterImpl topicFilter) {
    return matches(
        toBinary(), getFilterByteStart(), topicFilter.toBinary(), topicFilter.getFilterByteStart());
  }

  private static boolean matches(
      @NotNull final byte[] filter1,
      final int offset1,
      @NotNull final byte[] filter2,
      final int offset2) {
    ...
    while (i1 < filter1.length) {
      ...
      if (b1 == MULTI_LEVEL_WILDCARD) {
        ...
        if (i2 == filter2.length) {
          // lookahead for "/#" as it includes the parent level
          return (b1 == MqttTopic.TOPIC_LEVEL_SEPARATOR)
              && (i1 + 1 == filter1.length)
