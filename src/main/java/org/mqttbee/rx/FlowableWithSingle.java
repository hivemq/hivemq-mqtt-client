...
/**
 * A {@link Flowable} which emits a single item of type {@link S} and a flow of items of type {@link
 * F}.
 *
...
public abstract class FlowableWithSingle<S, F> extends Flowable<F> {
    ...
    /**
     * Modifies the upstream to perform its emissions and notifications including the single item on
     ...
     * {@link S} mapping it to an item of type {@link SM} and a a specified function to the flow of
     * items of type {@link F} mapping them to items of type {@link FM}.
     *
     ...
     * @return a {@link FlowableWithSingle} that applies the mapper functions to the single item and
     *     the flow of items.
     */
    @NotNull
    ...
    /**
     * Modifies the upstream so that it applies a specified function to an error which can map it to
     * a different error.
     *
    ...
    @BackpressureSupport(BackpressureKind.NONE)
    @SchedulerSupport(SchedulerSupport.NONE)
    public abstract FlowableWithSingle<S, F> mapError(
    ...
    @BackpressureSupport(BackpressureKind.NONE)
    @SchedulerSupport(SchedulerSupport.NONE)
    public abstract Flowable<F> doOnSingle(
            @NotNull final BiConsumer<S, Subscription> singleConsumer);
}
