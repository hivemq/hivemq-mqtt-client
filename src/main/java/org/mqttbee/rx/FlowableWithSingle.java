...
/**
 * A {@link Flowable} which emits a single item and a stream of items. It transforms an upstream
 * {@link Flowable} of type {@link T}, which is a supertype of {@link S} and {@link F}, to a single
 * item of type {@link S} and a stream of items of type {@link T}.
 *
 ...
 *       Publisher}'s backpressure behavior.
 *   <dt><b>Scheduler:</b>
...
public class FlowableWithSingle<T, S extends T, F extends T> extends Flowable<F> {
  ...
  public FlowableWithSingle(
      @NotNull final Flowable<? super T> source,
    ...
    if (s instanceof ConditionalSubscriber) {
      ...
      source.subscribe(
          new FlowableWithSingleConditionalSubscriber<>(
              cs, singleClass, flowableClass, singleConsumer));
    } else {
      source.subscribe(
          new FlowableWithSingleSubscriber<>(s, singleClass, flowableClass, singleConsumer));
    }
  }
  ...
  public FlowableWithSingle<T, S, F> observeOnWithSingle(@NotNull final Scheduler scheduler) {
    return new FlowableWithSingle<>(
        source.observeOn(scheduler), singleClass, flowableClass, singleConsumer);
  }
  ...
  /**
   * Modifies this {@link FlowableWithSingle} so that it invokes an action when the single item of
   * type {@link S} is emitted from the upstream.
   *
   * @param singleConsumer the consumer of the single item.
  ...
  private abstract static class FlowableWithSingleAbstractSubscriber<T, S extends T, F extends T>
    ...
    private FlowableWithSingleAbstractSubscriber(
        @NotNull final Subscriber<? super F> actual,
        @NotNull final Class<S> singleClass,
        @NotNull final Class<F> flowableClass,
        @Nullable final BiConsumer<S, Subscription> singleConsumer) {
    ...
    public void onSubscribe(final Subscription s) {
      ...
      if (s instanceof QueueSubscription) {
        @SuppressWarnings("unchecked")
        final QueueSubscription<T> qs = (QueueSubscription<T>) s;
        this.qs = qs;
      }
  ...
  private static final class FlowableWithSingleConditionalSubscriber<T, S extends T, F extends T>
    ...
    private FlowableWithSingleConditionalSubscriber(
        @NotNull final ConditionalSubscriber<? super F> actual,
        @NotNull final Class<S> singleClass,
        @NotNull final Class<F> flowableClass,
        @Nullable final BiConsumer<S, Subscription> singleConsumer) {
